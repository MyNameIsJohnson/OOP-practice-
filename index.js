// Meeting 
// Thomas 1:45pm -- 
// refactoring poorly writing code with test factor that doesnt occur regressions 
// Wont have enough time to refactor I have to weigh the time to refactor 
// Melisa -- technical deep dive 
// Zach -- 
// object design give eco system with new system object model what are there attribute and how they relate 
// Have to sudo code 
// Extend data schema cleanly to handle rule groups 
// Origanize to binary tree 
// Sudo code for data schema relation 
// Write complex rules 
// /******************
//  * Name Matching
//  *
...
function nameMatchCore(aliases_names, names){
  
  return	( aliases_names.length !== names.length 
            && aliases_names[0] == names[0] )  
            || ( aliases_names[1][0] == names[1] ) 

            || ( aliases_names[1] == names[1][0] )  

}

function transpositon(aliases){
  var first = aliases[0]
  var middle = aliases[1]
  var last = aliases[2]
  
	var	transposed = []

  transposed.push(middle)  
  transposed.push(first)
  transposed.push(last)
  
  return transposed
}

function nameMatch(knownAliases, name) {
  if (knownAliases.includes(name)){
    return true
  }
  
  for ( var i = 0; i < knownAliases.length; i++){
    
    var names = name.split(' ')    
    var aliases_names = knownAliases[i].split(' ')
		
    if (nameMatchCore(aliases_names, names)){
      
      return true 
    }
    
	  var transposed = transpositon(aliases_names)
    
    if (nameMatchCore(transposed, names)){	
